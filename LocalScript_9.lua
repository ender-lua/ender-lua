...
a_11.MouseButton1Click:Connect(function()
	...
	for i=1,#am do
		...
		local num=am:sub(i,i)
		table.insert(e,num)
	end
	local numm_1={}
	local numm_2={}
	local a=""
	for i,num in ipairs(e) do
		if tonumber(num) and stop == false then
			table.insert(numm_1,num)
		elseif tonumber(num) and stop==true then
			table.insert(numm_2,num)	
		else
			stop=true
			if num =="+" or num=="-" or num=="x" or num == "X" or num=="÷" or num=="2" then
				a=num
			end
		end
	end
